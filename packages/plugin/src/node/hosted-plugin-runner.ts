...
export const HostedPluginRunner = Symbol('HostedPluginRunner');

/**
 * Is responsible for running and handling separate Theia instance with given plugin.
 */
export interface HostedPluginRunner {
    /**
...
@injectable()
export abstract class AbstractHostedPluginRunner implements HostedPluginRunner {
    ...
    async run(pluginUri: URI, port?: number): Promise<URI> {
        ...
        this.instanceUri = await this.runHostedPluginTheiaInstance(command, processOptions);
...
@injectable()
export class NodeHostedPluginRunner extends AbstractHostedPluginRunner {
...
@injectable()
export class ElectronNodeHostedPluginRunner extends AbstractHostedPluginRunner {
    terminate(): void {
        if (this.isPluginRunnig) {
